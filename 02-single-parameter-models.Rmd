...
We set the prior alpha =200 and beta =10 because the effective sample size ($\beta$) is 10 years, and the prior mean is approximately 20 which is reasonably close to the table 2.2 values, although a sensitivity analysis is recommended.

We use the conjugate prior for Gamma to find the posterior distribution, which closely is approximated by the normal distribution.  The posterior distributions are similar.   The normal approximation prior followed $N(\alpha/\beta, \sqrt{\alpha}/\beta)$.  we set the variance to be known $\sigma^2=100$.   Using the gamma conjugate prior the predictive interval is $95\% $ (13,32).
...
# data entry
 ...
 ## the mean rate of deaths is 0.126
 ## we approximate a prior to have an average of 20 deaths across the decade.
   alpha= 200
   beta<- 10
...
mean(post.draws) ### posterior mean is arout 21.92
...
Using the normal approximation, the variance of the predictive posterior is $\sigma^2+\tau^2$ and is a larger interval (0.87, 40.4).   sampling values from the predictive posterior is much larger interval, but the predictive posterior mean is $\mu_n \approx 21$.  The Gamma conjugate prior has a smaller predictive interval but similar posterior.
```{r}
