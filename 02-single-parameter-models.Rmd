...
The average predictive value from 1986 is 23.9 which is close to the true value 22 fatal accidents.
```{r}
...
If we take the posterior fatal accident rate per 100,000 miles and multiple by the average miles (5716),  we have the observed fatal accidents as 22.1 (which is approximately 22). The posterior credible interval for average miles per 100,000 is (20.97, 26.97) which is close to the gamma interval of counts (21.1, 27).
...
lines(theta,dgamma(theta,alpha+sum(fatal),rate=beta+sum(miles)),col='red')
 ...
 ### i think i need to use the formuls  f(x | y ) to find the probability and sample there ?
  post.draws<-sample(theta,size=1000,prob=post_pois,replace=T)
...
- (c) we repeat part (a) but for passenger deaths.  we need to find a prior and determine the posterior distribution with a predictive interval.
 ...
 Note that the death rate mean is not equal to the variance so the performance is not as good compared to the fatal accidents.  The 1986 prediction is 692 (638, 750) which is not the expected prediction of 546 passenger deaths.  This is likely because Poisson is not a good fit because it is constrained.
```{r}
...
# a/b =  691  and a/b^2 = 63700.32,  we solve for a and b
 #  the beta parameter is set rate is set such that   404, 832 which matches well the observed data.
   ...
   ## the pr
   n<-nrow(data)
qgamma(c(0.025,0.795),alpha,beta)
...
lines(theta,dgamma(theta,alpha+n*mean(death),rate=beta+n))
 ...
 sum(theta*post_pois/sum(post_pois))
  ...
  ## prediction:  with the grid approach in the Gamma framework.  
...
```
- (d) Using the rate the posterior rate mean is 0.12 estimated from sampling the posterior distribution.  For 8 x 10$^11$ miles flown the deaths are 960. this is higher than the 1986 observed value.
...
#summary(miles)
 ...
 # miles*acc_rate  ## number of fatal accidents.
 print("passanger death rate per 100 million flown miles summary")
   summary(rate)
  ## crude rate 23.8 fatal accidents by 100 million miles driven.
...
lines(theta,dgamma(theta,alpha+sum(death),rate=beta+sum(miles)),col='red')
 ## credible interval of the posterior
 pois_credible<-qgamma(c(.025,0.975),alpha+sum(death),rate=beta+sum(miles))
 message("posterior gamma interval n=10: ",round(pois_credible[1],3)," ",round(pois_credible[2],3)) ## 

 
  post_pois<-dgamma(theta,alpha+sum(death),rate=beta+sum(miles))
